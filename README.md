...
- **`Version` header (REQUIRED, auto-fetched)**: `/api/v2/chat/completions` rejects requests that lack the frontend version header (`Version: 0.2.87`) with HTTP 200 JSON `{"code":"Bad_Request","details":"Internal error..."}` — chat creation and token validation do NOT need it, which makes this failure easy to misdiagnose. By default the proxy **scrapes the current version from chat.qwen.ai's homepage** (`qwen-chat-fe/<version>/` in the HTML), caches it for 30 min, refreshes in the background, and if a completion ever fails with that exact Bad_Request signature it force-refreshes and retries once — so Qwen frontend updates need zero manual action. Pin via `QWEN_FE_VERSION=0.2.87` (or `config.qwen.frontendVersion`) to disable scraping. Verified against the site's own JS bundle on 2026-08-24.
